...
var EVENTS = {
    ...
    return new WHS.Loop(function () {
        ...
        if (t.distanceTo(o) < e.basketGoalDiff && Math.abs(t.y - o.y + e.basketYDeep()) < e.basketYGoalDiff() && !APP.goal){
            APP.goal = true;
            // TODO: count points
            goal_counter++;
            counter_txt.innerText = goal_counter.toString();
            console.log("goal_counter: ",goal_counter)
        }
    })
}, APP = {
    helpersActive: !0,
...
//stats: "fps",
    ...
    createScene: function () {
        APP.ground = new WHS.Plane({
            ...
            mass: 0,
            material: {kind: "phong", color: APP.bgColor},
            pos: {y: -20, z: 120},
            rot: {x: -Math.PI / 2}
    ...
    addLights: function () {
        new WHS.PointLight({
            light: {distance: 100, intensity: 1, angle: Math.PI},
            shadowmap: {width: 1024, height: 1024, left: -50, right: 50, top: 50, bottom: -50, far: 80, fov: 90},
            pos: {y: 60, z: -40}
        }).addTo(APP.world).then(function () {
            return APP.ProgressLoader.step()
        }), new WHS.AmbientLight({light: {intensity: .3}}).addTo(APP.world).then(function () {
            return APP.ProgressLoader.step()
        })
    },
    addBasket: function () {
        APP.backboard = new WHS.Box({
            geometry: {buffer: !0, width: 41, depth: 1, height: 28},
            mass: 0,
            material: {
                kind: "standard",
            ...
            pos: {y: APP.basketY + 10, z: APP.getBasketZ() - APP.getBasketRadius()}
            ...
            pos: {y: APP.basketY, z: APP.getBasketZ()},
            ...
            softbody: !0,
            material: {
                map: WHS.texture("textures/net4.png", {repeat: {y: .7, x: 2}, offset: {y: .3}}),
                transparent: !0,
                opacity: .7,
                kind: "basic",
            ...
            pos: {y: APP.basketY - 8, z: APP.getBasketZ()}
    ...
    addBall: function () {
        APP.ball = new WHS.Sphere({
            ...
            material: {
                kind: "phong",
                map: WHS.texture("textures/ball3.png"),
                normalMap: WHS.texture("textures/ball3_normal.png"),
                bumpMap:WHS.texture("textures/ball3_normal.png"),
                shininess: 20,
                reflectivity: 2,
                normalScale: new THREE.Vector2(1.5, 1.5)
            },
            physics: {restitution: 3}
 ...
 APP.ball.applyCentralImpulse(t), t.multiplyScalar(10 / APP.force.m), t.y = t.x, t.x = APP.force.y, t.z = 0, APP.ball.setAngularVelocity(t), APP.thrown = !0//, APP.menu.attempts++
        }
    },
    keepBall: function () {
